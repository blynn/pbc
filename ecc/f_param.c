...
static void cc_miller_no_denom(element_t res, mpz_t q, element_t P,
  ...
  void f_miller_evalfn(void)
  {
    ...
    inline void do_term(int i, int j, int k, int flag)
    {
      ...
      element_mul(element_re(e1), element_re(element_item(v, i)), c);
      element_mul(element_im(e1), element_im(element_item(v, i)), c);
      element_add(e2, e2, e1);
    }
    do_term(0, 2, 3, 2);
    do_term(1, 3, 4, 2);
    ...
    element_set(element_re(element_item(e0, 0)), c);
...
void pbc_param_init_f_gen(pbc_param_t p, int bits) {
  ...
  element_to_mpz(fp->alpha0, element_re(element_item(f, 0)));
  element_to_mpz(fp->alpha1, element_im(element_item(f, 0)));
